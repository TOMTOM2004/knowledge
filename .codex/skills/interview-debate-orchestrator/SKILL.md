...
出力形式（内部処理用）:
```
[<persona_id>] 良い点: ..., 問題点: ..., 深掘り候補: <質問>（priority: 0.XX）
```
...
- 論理飛躍・矛盾の有無
- 差し込み質問の要否（`intervention_trigger.priority_threshold` 以上なら差し込み）
- 会社固有性の不足
...
`answer-reviewer` スキルのロジックに従い、4区分FBを生成する。

表示タイミング: 各質問の回答完了後に即時表示する（セッション中断を避けるためコンパクト版を出す）。
...
- 質問表示は1問ずつ（先読みさせない）
- 回答評価はユーザー入力後にのみ実行する（先行してペルソナ評価を出力しない）
- common_critic の差し込みは1セッションで最大3回まで
- セッション中に「スキップ」と入力された場合はその質問を記録してスキップする
- ランダム質問は必ず1問含める（questions.json に含まれている）
